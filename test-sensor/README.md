...
- `arduino/esp32_workout_firmware/esp32_workout_firmware.ino` (ESP32 เท่านั้น -- เฟิร์มแวร์หลักตัวเต็ม: WiFi + ปุ่ม 2 ปุ่ม + จอ LCD + บัซเซอร์ + รับค่า sEMG/FSR จาก Arduino Uno ผ่าน UART; ดูการต่อสายที่ [`PINS.md`](../PINS.md) ที่ root ของโปรเจกต์ ต้องติดตั้งไลบรารีเพิ่มอีก 1 ตัวคือ `LiquidCrystal_I2C` นอกจาก 4 ตัวด้านล่าง; สถาปัตยกรรมภายในเป็น FreeRTOS 4 tasks + hardware timer + interrupt ปุ่ม + task watchdog + light sleep ตอน idle นาน -- ดูรายละเอียดทั้งหมดที่ [`docs/firmware_rtos_power.md`](../docs/firmware_rtos_power.md))
- `arduino/uno_emg_fsr_link/uno_emg_fsr_link.ino` (Arduino Uno/Nano เท่านั้น -- อ่าน sEMG (A0) + FSR (A1) ที่ 100Hz แล้วส่งให้ ESP32 ผ่าน **hardware Serial (พิน 0/1)** ไม่ใช่ `SoftwareSerial`/พิน 2-3 (ห้ามต่อ USB พร้อมกับสาย ESP32 เพราะใช้พินร่วมกัน); ไม่ต้องติดตั้งไลบรารีเพิ่มเติม; สถาปัตยกรรมภายในใช้ Timer1 (CTC) + ADC interrupt-driven sampling + AVR watchdog + `sleep_mode()` (ไม่มี RTOS -- ชิป ATmega328P 2KB RAM ไม่รองรับ FreeRTOS ในทางปฏิบัติ) -- ดูรายละเอียดที่ [`docs/firmware_rtos_power.md`](../docs/firmware_rtos_power.md); **ยังไม่ได้ compile-verify** บนเครื่องพัฒนา arm64 ที่ไม่มี Rosetta 2 สำหรับ toolchain AVR ในรอบล่าสุด ทดสอบคอมไพล์ก่อน flash จริง)
